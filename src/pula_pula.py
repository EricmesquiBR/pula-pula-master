...
class PulaPula:
    ...
    def entrarNaFila(self, crianca: Crianca):
        if crianca not in self.filaDeEspera:
            for kids in self.filaDeEspera:
                if kids.getNome() == crianca.getNome():
                    return False
    ...
    def papaiChegou(self, nome):
        if self.criancasPulando:
            for kids in self.criancasPulando:
                if kids.getNome() == nome:
    ...
    def fechar(self):
        if self.criancasPulando:
            for kids in self.criancasPulando:
                self.caixa += self.conta[kids.getNome()]
                self.conta[kids.getNome()] = 0
            self.criancasPulando = []
            if self.filaDeEspera:
                for kids in self.filaDeEspera:
                    self.caixa += self.conta[kids.getNome()]
